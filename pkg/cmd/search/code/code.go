...
func codeRun(opts *CodeOptions) error {
	...
	if len(result.Items) == 0 && opts.Exporter == nil {
		return cmdutil.NewNoResultsError("no code matched your search")
	}
...
func displayResults(io *iostreams.IOStreams, now time.Time, results search.CodeResult) error {
	if now.IsZero() {
		now = time.Now()
	}
	//cs := io.ColorScheme()
	tp := tableprinter.New(io)
	tp.HeaderRow("Repo", "SHA", "Filename", "Path", "Score")
	for _, code := range results.Items {
		tp.AddField(code.Repo.FullName)
		tp.AddField(code.Sha)
		tp.AddField(code.Name)
		tp.AddField(code.Path)
		//tp.AddField(text.RemoveExcessiveWhitespace(code.Info.Message))
		//tp.AddField(code.Author.Login)
		//tp.AddTimeField(now, code.Info.Author.Date, cs.Gray)
		tp.AddField(code.Score)
		tp.EndRow()
	}
	if io.IsStdoutTTY() {
